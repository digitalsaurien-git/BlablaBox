import { MSG_TIMEOUT_INTERNAL, MSG_UNVERIFIABLE_INTERNAL, MSG_COURSE_READING_REQUIRED } from '../messages.ts';
...
export class LearningFailure extends Error {
  ...
  constructor(code:LearningErrorCode) {
    super(code==='TIMEOUT'?MSG_TIMEOUT_INTERNAL:MSG_UNVERIFIABLE_INTERNAL);
...
export function learningErrorCode(error:unknown):LearningErrorCode {
  ...
  if(message===MSG_TIMEOUT_INTERNAL)return 'TIMEOUT';
  if(message===MSG_COURSE_READING_REQUIRED)return 'SOURCE_UNUSABLE';
