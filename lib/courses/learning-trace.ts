...
const EVENTS=['action-start','generation-start','generation-end','validation-start','validation-end','audit-start','audit-end','transaction-start','done','failed'] as const;
...
export function createLearningTrace():LearningTrace {
  ...
  function emit(event:Event,metadata:Metadata={}) {
    ...
    if(LEARNING_ERROR_CODES.some(c=>c===metadata.errorCode))safe.errorCode=metadata.errorCode;
