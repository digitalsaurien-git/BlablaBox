...
import { LEARNING_ERROR_CODES, learningErrorCode, safeUsage, failureUsage, failureDiagnostics, safeDiagnostics, type TokenUsage, type LearningFailureDiagnostics } from './learning-errors.ts';
...
type Metadata=TokenUsage & LearningFailureDiagnostics & {totalBlocks?:number;acceptedBlocks?:number;rejectedBlocks?:number;errorCode?:string};
...
export function createLearningTrace():LearningTrace {
  ...
  function emit(event:Event,metadata:Metadata={}) {
    ...
    Object.assign(safe,safeDiagnostics(metadata));
  ...
  return {
    ...
    failed(error) {
      emit('failed',{errorCode:learningErrorCode(error),...failureUsage(error),...failureDiagnostics(error)});
    },
  };
}
