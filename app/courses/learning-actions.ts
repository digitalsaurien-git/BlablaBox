...
export async function startCourseLearning(form:FormData) {
  ...
  redirect(result.sessionId?`${coursePath(id)}/session/${result.sessionId}`:mode==='flashcards'?`${coursePath(id)}/flashcards/${result.versionId}`:mode==='mindmap'?`${coursePath(id)}/mind-map/${result.versionId}`:`${coursePath(id)}/content/${result.versionId}`);
}
