...
import { BackToCourse, LearningError, learningButton, learningPanel } from '@/components/learning-ui';
import { confirmCourseReading } from '../../../learning-actions';
export default async function ReadingPage({params,searchParams}:{params:Promise<{id:string;extractionId:string}>;searchParams:Promise<{error?:string}>}) {
  const {error}=await searchParams;const user=await requireCurrentUser();const {id,extractionId}=await params;
