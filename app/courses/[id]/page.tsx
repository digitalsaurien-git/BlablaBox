...
export default async function CoursePage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{readingError?:string}>}) {
  ...
  const versions=await prisma.projectVersion.findMany({where:{userId:user.id,courseThemeId:id,mode:{in:['explain','summary','essential','memo','visual']}},orderBy:{createdAt:'desc'},take:10,select:{id:true,mode:true,createdAt:true}});
  ...
  {versions.length||sessions.length?<section className={learningPanel}><h2 className="text-lg font-bold">Reprendre mon travail</h2>{versions.map(v=><Link className="text-moss underline" key={v.id} href={`/courses/${id}/content/${v.id}`}>{({explain:'Explication',summary:'Résumé',essential:'Essentiel',memo:'Fiche mémo',visual:'Visuel'} as Record<string,string>)[v.mode]} · {v.createdAt.toLocaleDateString('fr-FR')}</Link>)}{sessions.map(s=><Link className="text-moss underline" key={s.id} href={`/courses/${id}/session/${s.id}`}>{s.kind==='revision'?'Reprendre une révision':'Reprendre un devoir'}</Link>)}</section>:null}
