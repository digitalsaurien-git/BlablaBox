...
export default async function CourseContent({params,searchParams}:{params:Promise<{id:string;versionId:string}>;searchParams:Promise<{error?:string}>}) {
  ...
  const omissionNotice=output.elementsOmitted?<p role="status" className="rounded-xl bg-mist p-3 text-sm">Certains éléments ont été écartés. Voici ceux que nous avons pu vérifier.</p>:null;
