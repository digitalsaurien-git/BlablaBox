...
export default async function CourseContent({params,searchParams}:{params:Promise<{id:string;versionId:string}>;searchParams:Promise<{error?:string}>}) {
  ...
  const version=await prisma.projectVersion.findFirst({where:{id:versionId,userId:user.id,courseThemeId:id,mode:{in:['explain','summary','essential','memo','visual']}}});if(!version)notFound();
